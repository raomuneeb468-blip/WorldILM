...
// We will just do a regex!
// Find the exact string from tmp.txt
// It is `{m.content}\n                          </div>                          </div></div>) : (`
content = content.replace(/\{m\.content\}\n\s*<\/div>\s*<\/div><\/div>\) : \(/, '{m.content}\n                          </div>\n                          </div></div></div>) : (');

// Wait! If I just replace it with my new copyEditButtons:
...
const titleReplacement = `<span className="text-xs font-extrabold text-zinc-800 dark:text-zinc-100 uppercase tracking-wider font-sans leading-none">
                              ...
                              {isBusy && idx === messages.length - 1 && (
                                <motion.span
                                  ...
                                  className="text-[9px] text-indigo-500 font-extrabold uppercase tracking-widest mt-1 flex items-center h-3 overflow-hidden"
                                >
